...
public class EliteCustomer extends Customer {
    ...
    public EliteCustomer(String username, String password, Admin admin, float balance, Cart cart) {
        super(username, password, admin);
        this.status = "elite";
        this.userTypeDiscount = 10;
    ...
    public int checkout() {
        ...
        for (Map.Entry<Float, Product> set: cartProducts.entrySet()) {
            ...
            if (discountType == 0) {
                ...
                priceWithDiscount = getDiscountedPrice(price, product.eliteDiscount);
        ...
        for (Map.Entry<Integer, Deal> set: cartDeals.entrySet()) {
            ...
            System.out.println("Combo deal price = " + deal.elitePrice + "\n");
            totalWithDiscount += deal.elitePrice;
        }
    ...
    private int bestDiscount(Product product, float bestCoupon) {
        ...
        discounts.add(this.userTypeDiscount); discounts.add(product.eliteDiscount); discounts.add(bestCoupon);
        if (this.userTypeDiscount == Collections.max(discounts)) {
            return 0; // user type discount
        } else if (product.eliteDiscount == Collections.max(discounts)) {
    ...
    private void generateCoupons() {
        ...
        int howMany = 3 + random.nextInt(2);
